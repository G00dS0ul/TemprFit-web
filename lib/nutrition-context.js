...
export async function buildNutritionContext(user, profile) {
  ...
  if (profile) {
    ...
    if (profile.budget) lines.push(`Budget constraint: ${profile.budget} (strictly adhere to this)`)
    if (profile.cookingSkill) lines.push(`Cooking skill level: ${profile.cookingSkill} (tailor recipe complexity to this)`)
    if (profile.cookingTime) lines.push(`Time available for cooking: ${profile.cookingTime} (do not suggest meals that take longer)`)
    if (profile.pantry && profile.pantry.length) {
      lines.push(`AVAILABLE PANTRY / INGREDIENTS (STRICT INSTRUCTION: Center the meals around these ingredients as much as possible to save money and reduce waste): ${profile.pantry.join(', ')}`)
    }
  } else {
