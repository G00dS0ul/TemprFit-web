 ...
 */
export async function askGemini({ systemPrompt, history = [], userMessage, attachment, attachments, responseMimeType }) {
  ...
  // Vision capabilities need the pro or flash models
  const model = allAttachments.length > 0 ? 'gemini-3.5-flash' : (process.env.GEMINI_MODEL || DEFAULT_MODEL)
  console.log(`[Gemini] Requesting ${model} (key present, ${apiKey.length} chars)`)

  const userParts = [{ text: userMessage || 'Analyze this attachment.' }];
