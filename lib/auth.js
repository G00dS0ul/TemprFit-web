...
/**
 * Shared helper to verify administrator access across admin routes and layouts.
 * Checks:
 * 1. Logged-in database user with role === 'admin'
 * 2. admin_token cookie or Bearer token carrying signed admin JWT (role === 'admin' || isAdmin === true)
 * 3. Legacy admin_token === 'true'
 *
 * @returns {Promise<{ authorized: boolean, user: object|null, isMasterAdmin: boolean, tokenPayload?: object }>}
 */
export async function verifyAdminRequest() {
  ...
  // 1. Check if authenticated user session is a database admin
  const sessionUser = await getSessionUser()
  if (sessionUser && sessionUser.role === 'admin') {
    return { authorized: true, user: sessionUser, isMasterAdmin: false }
  }

  // 2. Extract admin token from cookie or Authorization header
  let adminToken = reqCookies.get('admin_token')?.value
  if (!adminToken) {
    const authHeader = reqHeaders.get('authorization')
    if (authHeader && authHeader.startsWith('Bearer ')) {
  ...
  if (adminToken) {
    if (adminToken === 'true') {
      return { authorized: true, user: sessionUser || null, isMasterAdmin: true }
    }

    const payload = verifyToken(adminToken)
    if (payload && (payload.role === 'admin' || payload.isAdmin)) {
      const revoked = await isTokenRevoked(adminToken)
      if (!revoked) {
        return {
          authorized: true,
  ...
  return { authorized: false, user: null, isMasterAdmin: false }
}
